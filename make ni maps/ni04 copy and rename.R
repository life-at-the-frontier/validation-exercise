## 04 could we simply just copy and rename the maps we use 
...
thisMapOrder <-
  "cleaned data/ni map order 2021-10-07.rds"
...
setTable %>% 
  ...
  write.csv(
    file.path(trialMaps_dir, 'ni fillable results sheet.csv'),
    row.names = F
  )

## save the map order original name 
cat(thisMapOrder, 
    file = file.path(trialMaps_dir, "ni order ID.txt")
    )
