...
pair_1 <-
  bind_rows(
    mapdataList$a %>% mutate(type = map_a_position[1], pair = 1),
    mapdataList$b %>% mutate(type = map_b_position[1], pair = 1),
  )

pair_2 <-
  bind_rows(
    mapdataList$a %>% mutate(type = map_a_position[2], pair = 2),
    mapdataList$c %>% mutate(type = map_b_position[2], pair = 2),
  )
